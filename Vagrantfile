...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "client-a1" do |client_a1|
    ...
    # Install dependencies and define the NAT
    client_a1.vm.provision :shell, run: "always", path: "scripts/client.sh", :args => ["acfdd8113f1f4e1781d407eddfb7ef96","ba8d0ecc8f2140f184f20604c59f74ca", "10.0.0.1/24","xe0p8hnlRy5lTtucZcj-HA","77U9Nt_mG8xOE81hmALaFQ"]
  end
  ...
  config.vm.define "client-a2" do |client_a2|
    ...
    client_a2.vm.provision :file, source: './apps/privates/privkey2.key',
      destination: "overlay_manager/private.key"
    # Install dependencies and define the NAT
    client_a2.vm.provision :shell, run: "always", path: "scripts/client.sh", :args => ["acfdd8113f1f4e1781d407eddfb7ef96", "7be0479e124f42ff9e435b2656431f66", "10.0.0.2/24","0uAmWjBp6GSe9AT6Q260Mg"]
  end
  ...
  config.vm.define "client-b1" do |client_b1|
    ...
    client_b1.vm.provision :file, source: './apps/privates/privkey4.key',
      destination: "overlay_manager/private.key"
    # Install dependencies and define the NAT
    client_b1.vm.provision :shell, run: "always", path: "scripts/client2.sh", :args => ["53aaa3af93c04a83aff95d674c4b9743", "780c397dcdf74bda88ce7ffdb3c45a01", "10.0.1.1/24","Tx--I72aeWE_k_jrpUwiZw"]
  end
  ...
  config.vm.define "client-b2" do |client_b2|
    ...
    client_b2.vm.provision :file, source: './apps/privates/privkey5.key',
      destination: "overlay_manager/private.key"
    # Install dependencies and define the NAT
    client_b2.vm.provision :shell, run: "always", path: "scripts/client2.sh", :args => ["53aaa3af93c04a83aff95d674c4b9743", "9afccfc8aeb54afd982f489d325c5c08", "10.0.1.2/24","grA4HtiwvBwGagO1yHGN_Q"]
  end
  ...
  config.vm.define "server-s1" do |server_s1|
    ...
    server_s1.vm.provision :file, source: './apps/privates/privkey3.key',
      destination: "overlay_manager/private.key"
    # Install dependencies and define the NAT
    server_s1.vm.provision :shell, run: "always", path: "scripts/cloud_server.sh", :args => ["acfdd8113f1f4e1781d407eddfb7ef96", "f1f0b43ee8dd48eaa8385ed079658b46", "10.0.0.3/24","fQYBGjuQHw8gNxDTMjN46Q"]
  end
  ...
  config.vm.define "server-s2" do |server_s2|
    ...
    # Install dependencies and define the NAT
    server_s2.vm.provision :shell, run: "always", path: "scripts/cloud_server.sh", :args => ["53aaa3af93c04a83aff95d674c4b9743", "79fff66041014b56bcf71687e9b4d3a5", "10.0.1.3/24", "hfQ5Tb_peKk9DzLodcNFlA"]
  end
